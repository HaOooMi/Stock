...
2. 生成通用技术分析特征（价格、成交量、技术指标）
3. 特征选择和优化（方差过滤、相关性去除、重要性排序）
4. 特征标准化（RobustScaler防泄漏）

【与因子工厂的关系】
- feature_engineering.py（本文件）：通用特征工程，适用于任何股票ML任务
- factor_factory.py：专注于「高信息量因子」的批量生成（4大因子族40+因子）
- factor_quality_checker.py：6层质量检查（IC/ICIR、衰减、分布稳定性）
- factor_library_manager.py：因子版本管理、入库标准、清单维护

【使用场景】
- 本模块：快速原型、单股票回测、通用ML pipeline
- 因子工厂：多股票横截面、因子挖掘、严格质量控制

【接口兼容性】
两个模块完全兼容，输出格式均为 MultiIndex[date, ticker]，可无缝切换。

最后更新: 2025-01-20 (因子工厂v1上线)
"""
